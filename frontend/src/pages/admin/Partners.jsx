...
import { getAdminPartners, updatePartnerRegistrationStatus } from '../../api/admin/partners'
...
const AdminPartners = () => {
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      ...
      setPartners(prev => prev.map(p => p.id === id ? { ...p, registration_status: newStatus } : p))
      if (selectedPartner && selectedPartner.id === id) {
        setSelectedPartner(prev => ({ ...prev, registration_status: newStatus }))
      }

      // Call API
      await updatePartnerRegistrationStatus(id, newStatus)
    } catch (err) {
      console.error(err)
      const msg = err?.response?.data?.message || err.message
      alert(`Failed to update status on server: ${msg}`)
      // Rollback
      fetchPartners()
    }
  }
    ...
    <>
      ...
      <div className="space-y-6 select-none animate-fade-in text-slate-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200/50 pb-4.5 gap-3">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-800">Manage Partners</h1>
            <p className="text-sm font-semibold text-slate-400 mt-1">
              Review and approve partner applications, view registered documents, and track compliance.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="relative flex-grow w-full">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-[#38b34a] focus:ring-2 focus:ring-[#38b34a]/10 transition-all"
            />
          </div>
                  ...
                  filteredPartners.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => { setSelectedPartner(p); setActiveTab('info'); setDocumentViewer(null) }}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl z-10 flex flex-col max-h-[90vh]">
              ...
              <div className="flex gap-4 border-b border-slate-100 my-4 shrink-0">
                <button
                  onClick={() => { setActiveTab('info'); setDocumentViewer(null) }}
                  className={`pb-2.5 text-xs font-bold uppercase tracking-wider border-b-2 transition ${activeTab === 'info' ? 'border-[#38b34a] text-[#38b34a]' : 'border-transparent text-slate-400 hover:text-slate-600'
                    }`}
                >
                  Registration Info
                </button>
                <button
                  ...
                  className={`pb-2.5 text-xs font-bold uppercase tracking-wider border-b-2 transition ${activeTab === 'docs' ? 'border-[#38b34a] text-[#38b34a]' : 'border-transparent text-slate-400 hover:text-slate-600'
                    }`}
                >
                  Submitted Documents
                </button>
                ...
                {activeTab === 'info' ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3.5">
                      {[
                        { key: 'pan_card', label: 'PAN Card Doc' },
                        { key: 'id_proof', label: 'ID Proof Doc' },
                        { key: 'organization_proof', label: 'Organization Proof' },
                        { key: 'signed_agreement', label: 'Signed Contract PDF' }
                      ].map((doc) => {
                        ...
                        return (
                          ...
                          >
                            <div>
                              ...
                              <span className="text-xs text-slate-700 font-mono mt-0.5 block truncate max-w-[180px]">{filename || 'Not uploaded'}</span>
                            </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:justify-between items-center gap-3.5 shrink-0">
                <div className="flex gap-2">
                  ...
                  {selectedPartner.registration_status !== 'suspended' && (
                    <button
                      onClick={() => handleStatusUpdate(selectedPartner.id, 'suspended')}
                  ...
                  {selectedPartner.registration_status !== 'pending' && (
                    <button
                      onClick={() => handleStatusUpdate(selectedPartner.id, 'pending')}
                      className="px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 text-xs font-bold transition cursor-pointer"
                    >
                      Set to Pending
                    </button>
                  )}
