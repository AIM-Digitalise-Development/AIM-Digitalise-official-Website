...
const EmployeeModal = ({ isOpen, onClose, onSave, editingEmployee, departments, designations, loading }) => {
  ...
  const [sameAsCurrent, setSameAsCurrent] = useState(() => {
    if (editingEmployee && editingEmployee.current_address && editingEmployee.current_address === editingEmployee.permanent_address) {
      return true
    }
    return false
  })
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingEmployee) {
        ...
        setSameAsCurrent(
          !!(editingEmployee.current_address && editingEmployee.current_address === editingEmployee.permanent_address)
        )
      } else {
        setForm({ ...EMPTY_FORM })
  ...
  const update = (field, value) => {
    setForm((prev) => {
      ...
      if (field === 'current_address' && sameAsCurrent) {
                ...
                <div className="px-3 pb-4 space-y-3">
                  ...
                  <div className="flex items-center gap-2 px-1 py-1">
                    <input
                      ...
                      className="w-4 h-4 rounded text-[#38b34a] border-slate-300 focus:ring-[#38b34a]/30 cursor-pointer"
                    />
                    <label htmlFor="same_as_current" className="text-xs font-bold text-slate-500 cursor-pointer select-none">
                  ...
                  <FormField label="Permanent Address">
                    <textarea
                      ...
                      placeholder={sameAsCurrent ? "Same as current address" : "Permanent address (if different)"}
                      rows="2"
                      disabled={sameAsCurrent}
                      className={`${inputClass} resize-none ${sameAsCurrent ? 'bg-slate-100 text-slate-400 cursor-not-allowed border-slate-200' : ''}`}
                    />
                  </FormField>
                </div>
              )}
            ...
            <div className="border border-slate-100 rounded-xl overflow-hidden">
              ...
              {openSections.identity && (
                <div className="px-3 pb-4 space-y-3">
                  ...
                  {/* Document Uploads (Static) */}
                  <div className="border-t border-slate-100 pt-3">
                    <span className="text-[10px] font-bold text-[#1e3e6b] uppercase tracking-wider block mb-2">Upload Identity & Bank Documents (Optional / Static)</span>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <FormField label="Aadhar Card">
                        <div className="relative">
                          <input
                            type="file"
                            onChange={(e) => console.log('Aadhar uploaded', e.target.files[0])}
                            className="w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200 border border-slate-200 rounded-xl p-2 bg-slate-50 cursor-pointer"
                          />
                          <p className="text-[9px] text-slate-400 font-bold mt-1">Upload front & back merged PDF/Image (Max 5MB)</p>
                        </div>
                      </FormField>
                      <FormField label="PAN Card">
                        <div className="relative">
                          <input
                            ...
                            className="w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200 border border-slate-200 rounded-xl p-2 bg-slate-50 cursor-pointer"
                          />
                          <p className="text-[9px] text-slate-400 font-bold mt-1">Upload clear copy of PAN PDF/Image (Max 5MB)</p>
                        </div>
                      </FormField>
                      <FormField label="Bank Pass Book">
                        <div className="relative">
                          <input
                            type="file"
                            onChange={(e) => console.log('Passbook uploaded', e.target.files[0])}
                            className="w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200 border border-slate-200 rounded-xl p-2 bg-slate-50 cursor-pointer"
                          />
                          <p className="text-[9px] text-slate-400 font-bold mt-1">Upload first page with details or statement (Max 5MB)</p>
                        </div>
                      </FormField>
                    </div>
                  </div>
                    ...
                    <FormField label="Monthly Salary (₹)" required>
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={!form.current_salary || !form.first_name || !form.last_name || !form.designation_id}
                          className="px-4 bg-blue-50 hover:bg-blue-100 disabled:opacity-40 disabled:hover:bg-blue-50 text-blue-600 border border-blue-100 hover:border-blue-200 rounded-xl font-bold text-xs flex items-center gap-1 cursor-pointer transition-all shadow-sm shrink-0"
                          title={(!form.current_salary || !form.first_name || !form.last_name || !form.designation_id) ? "Fill Name, Designation & Salary to generate letter" : "Generate Offer Letter PDF"}
                        >
                          📄 Letter
                        </button>
                      </div>
            ...
            <div className="border border-slate-100 rounded-xl overflow-hidden">
              ...
              {openSections.previous && (
                <div className="px-3 pb-4 space-y-3">
                  ...
                  {/* Document Uploads (Static) */}
                  <div className="border-t border-slate-100 pt-3">
                    <span className="text-[10px] font-bold text-[#1e3e6b] uppercase tracking-wider block mb-2">Upload Previous Employment Documents (Optional / Static)</span>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FormField label="Previous Offer Letter">
                        <div className="relative">
                          <input
                            type="file"
                            onChange={(e) => console.log('Prev offer letter uploaded', e.target.files[0])}
                            className="w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200 border border-slate-200 rounded-xl p-2 bg-slate-50 cursor-pointer"
                          />
                          <p className="text-[9px] text-slate-400 font-bold mt-1">Upload copy of offer/appointment letter (Max 5MB)</p>
                        </div>
                      </FormField>
                      <FormField label="3 Month Salary Slip">
                        <div className="relative">
                          <input
                            type="file"
                            onChange={(e) => console.log('Salary slips uploaded', e.target.files[0])}
                            className="w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200 border border-slate-200 rounded-xl p-2 bg-slate-50 cursor-pointer"
                          />
                          <p className="text-[9px] text-slate-400 font-bold mt-1">Upload combined copy of last 3 payslips (Max 8MB)</p>
                        </div>
                      </FormField>
                    </div>
                  </div>
